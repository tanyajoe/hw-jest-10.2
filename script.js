...
function calculateMaxScore(students) {
  let allStudents = [];
  for (let i = 0; i < students.length; i++) allStudents.push(...students[i]);
  ...
  for (let i = 0; i < allStudents.length; i++) {
    if (allStudents[i].score == maxScore) {
      result.push(
        `Поздравляем ${allStudents[i].name} с максимальным количеством баллов - ${allStudents[i].score}`
      );
    }
  }
  return result;
...
console.log(calculateMaxScore(students));

module.exports = calculateMaxScore;
